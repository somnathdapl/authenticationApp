...
module.exports.logBodyAndUploadedFile = (req, res, next) => {
  global.commonFunction.customLog("logBodyAndUploadedFile called");

  try {
    global.commonFunction.customLog("req.originalUrl", req.originalUrl);

    global.commonFunction.customLog("global.currentType", global.currentType);
    global.commonFunction.customLog("global.currentUser", global.currentUser);

    global.commonFunction.customLog('\x1b[95mBody:\x1b[0m');
    global.commonFunction.customLog(JSON.stringify(req.body, null, '\t').split('\n').forEach(line => console.log('\x1b[97m' + line + '\x1b[0m'))); // needed for multi-line coloring

    global.commonFunction.customLog('\x1b[95mFiles:\x1b[0m');
    global.commonFunction.customLog(req.files);

    next();

  } catch (e) {
    global.commonFunction.customLog("logBodyAndUploadedFile", e);
    next();
  }
